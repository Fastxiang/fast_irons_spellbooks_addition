...
public class ExplosionProjectile extends AbstractMagicProjectile {
    public ExplosionProjectile(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.setNoGravity(true);
    }

    public ExplosionProjectile(Level pLevel, LivingEntity pShooter) {
        this(FastEntityRegistry.EXPLOSION_PROJECTILE.get(), pLevel);
...
//            Vec3 motion = Utils.getRandomVec3(0.7).add(vec3.normalize()).scale(0.25);
    ...
    protected void onHit(HitResult hitResult) {
        if (!this.level().isClientSide) {
            ...
            playSound(SoundEvents.GENERIC_EXPLODE, 4.0F, (1.0F + (level.random.nextFloat() - level.random.nextFloat()) * 0.2F) * 0.7F);
